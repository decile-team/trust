...
class OrganDataset(Dataset):
    def __init__(self, data, root='/mnt/data2/akshit/data/organmnist/a/', transform=None):
        self.root = root
...
class OOD2Dataset(Dataset):
    def __init__(self,type='train',root='/mnt/data2/akshit/data/organmnist/a/ood-2/',transform=None):
        self.transform=transform
        images=[]
        targets=[]
        class_id=11
        for img in os.listdir(root+type):
            image = cv2.imread(os.path.join(root+type,img))
...
def load_dataset_custom(datadir, feature, split_cfg, augVal=False, dataAug=True):
    ...
    ood_train = OOD2Dataset('train',transform=id_training_transform)
