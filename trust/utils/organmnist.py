...
class OrganDataset(Dataset):
    def __init__(self, data, root='/mnt/data2/akshit/DIAGNOSE/data/organmnist/a/', transform=None):
        self.root = root
...
def create_ood_data(fullset, testset, split_cfg, num_cls, augVal):
    ...
    for i in range(num_cls): #all_classes
        ...
        if(i in selected_classes):
            ...
            remain_idx = list(set(remain_idx) - set(class_val_idx))
            class_lake_idx = list(np.random.choice(np.array(remain_idx), size=split_cfg['per_ood_lake'], replace=True)) #many ood samples in lake
...
def load_dataset_custom(datadir, feature, split_cfg, augVal=False, dataAug=True):
    
    num_cls = 12
    path = '/mnt/data2/akshit/DIAGNOSE/data/organmnist/a/'
    ...
    test_data = np.load(f'{path}test_balanced.npz', allow_pickle=True)
    ...
    # are retrieved.
    organ_full_train = OrganDataset(data=train_data, transform=id_training_transform)
    organ_test = OrganDataset(data=test_data, transform=id_test_transform)
    ood_train = OOD2Dataset(data=train_data,transform=id_training_transform)
