...
## return id_cls_idx
def create_ood_data_3(fullset, testset, split_cfg, num_cls, augVal):
...
def load_dataset_custom_3(datadir, feature, split_cfg, augVal=False, dataAug=True):
    ...
    if(feature=="ood"):
        train_set, val_set, test_set, lake_set, id_cls_idx = create_ood_data_3(fullset, test_set, split_cfg, num_cls, augVal)
        print("Custom dataset stats: Train size: ", len(train_set), "Val size: ", len(val_set), "Lake size: ", len(lake_set), "Test set: ", len(test_set))
        return train_set, val_set, test_set, lake_set, id_cls_idx, split_cfg['num_cls_idc']
